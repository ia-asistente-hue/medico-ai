...
export default function LoginPage() {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    if (resetError) {
      ...
      console.error('Mensaje:', resetError.message)
      console.error('Nombre:', resetError.name)
      console.error('Status:', (resetError as any).status)
      console.error('Objeto JSON:', JSON.stringify(resetError, Object.getOwnPropertyNames(resetError), 2))
      console.error('-----------------------------------')
      
      setError(resetError.message || 'Ocurrió un error al enviar el correo de recuperación.')
    } else {
      setSuccessMessage('Hemos enviado un enlace de recuperación a tu correo electrónico.')
    }
  }
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 sm:p-8 shadow-xl shadow-slate-200/60 border border-slate-100">
        ...
        <div className="mb-6 sm:mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="text-lg sm:text-xl font-semibold text-slate-800">
            {isForgotPassword ? 'Recuperar Contraseña' : 'Acceso Médico'}
          </h1>
          <p className="mt-1 text-xs text-slate-500">
            {isForgotPassword 
              ? 'Ingresa tu correo para restablecer tus credenciales' 
        ...
        {successMessage && (
          <div className="mb-5 flex items-center gap-2 rounded-xl bg-emerald-50 border border-emerald-200 p-3.5 text-xs text-emerald-700">
