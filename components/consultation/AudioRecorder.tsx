'use client';

import React, { useState } from 'react';

interface AudioRecorderProps {
  isRecording: boolean;
  isPaused: boolean;
  recordingSeconds: number;
  maxSeconds: number;
  loading: boolean;
  onStartRecording: () => void;
  onStopRecording: () => void;
  onPauseRecording: () => void;
  onResumeRecording: () => void;
  patientName?: string;
  patientDob?: string;
}

export default function AudioRecorder({
  isRecording,
  isPaused,
  recordingSeconds,
  maxSeconds,
  loading,
  onStartRecording,
  onStopRecording,
  onPauseRecording,
  onResumeRecording,
  patientName = 'Maria Lopez Perez',
  patientDob = '1980-04-12',
}: AudioRecorderProps) {
  // Estado local para evitar clics accidentales en el botón de finalizar
  const [confirmingStop, setConfirmingStop] = useState(false);

  // Iniciales del paciente
  const getInitials = (name: string) => {
    return name
      .split(' ')
      .map((n) => n[0])
      .slice(0, 2)
      .join('')
      .toUpperCase();
  };

  // Formato MM:SS
  const formatTime = (totalSecs: number) => {
    const mins = Math.floor(totalSecs / 60);
    const secs = totalSecs % 60;
    return `${mins.toString().padStart(2, '0')}:${secs.toString().padStart(2, '0')}`;
  };

  // Validación de tiempo cercano al límite (ej. últimos 60 segundos)
  const isNearLimit = maxSeconds - recordingSeconds <= 60;

  const handleStopClick = () => {
    if (!confirmingStop) {
      setConfirmingStop(true);
      // Si pasan 3 segundos y no confirma, se quita el estado de advertencia
      setTimeout(() => setConfirmingStop(false), 3000);
    } else {
      setConfirmingStop(false);
      onStopRecording();
    }
  };

  return (
    <div className="w-full max-w-4xl mx-auto rounded-3xl bg-white p-6 sm:p-8 shadow-sm border border-slate-100 font-sans">
      
      {/* TARJETA DEL PACIENTE */}
      <div className="flex items-center justify-between rounded-2xl bg-[#F8FAFC] p-4 mb-6">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#DCE8FF] text-[#0052FF] font-bold text-sm">
            {getInitials(patientName)}
          </div>
          <div>
            <span className="text-[10px] font-bold tracking-wider text-slate-400 uppercase block">
              PACIENTE ACTIVO
            </span>
            <h2 className="text-base font-bold text-slate-900">{patientName}</h2>
          </div>
        </div>
        <div className="text-right">
          <span className="text-[11px] font-medium text-slate-400 block">F. Nacimiento</span>
          <span className="text-xs font-semibold text-slate-800">{patientDob}</span>
        </div>
      </div>

      {/* ESTADO 1: CARGANDO / PROCESANDO CON IA */}
      {loading ? (
        <div className="rounded-2xl border border-dashed border-slate-200 p-12 text-center flex flex-col items-center justify-center space-y-4">
          <div className="inline-block h-12 w-12 animate-spin rounded-full border-4 border-[#0052FF] border-t-transparent"></div>
          <h3 className="text-base font-bold text-slate-800">Procesando nota médica...</h3>
          <p className="text-xs text-slate-400">MedikAI está estructurando la consulta en formato SOAP.</p>
        </div>
      ) : !isRecording ? (
        
        /* ESTADO 2: INICIAL / LISTO PARA GRABAR */
        <div className="rounded-2xl border border-dashed border-slate-200 p-12 text-center flex flex-col items-center justify-center">
          <div className="relative mb-6 flex items-center justify-center">
            <div className="h-32 w-32 rounded-full bg-[#EEF4FF] flex items-center justify-center">
              <button
                onClick={onStartRecording}
                className="flex h-20 w-20 items-center justify-center rounded-full bg-[#0052FF] text-white shadow-lg shadow-blue-500/20 hover:scale-105 transition-all"
              >
                <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z" />
                  <path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z" />
                </svg>
              </button>
            </div>
          </div>

          <h3 className="text-lg font-bold text-slate-900 mb-1">Iniciar Toma de Notas Médicas</h3>
          <p className="text-xs text-slate-400 max-w-sm mb-3">
            Dicta la anamnesis, exploración física y tratamiento en lenguaje natural.
          </p>
          
          <span className="text-[11px] text-slate-400 font-medium bg-slate-50 px-3 py-1 rounded-full border border-slate-100">
            Límite máximo recomendado: 10:00 min
          </span>
        </div>
      ) : (
        
        /* ESTADO 3: GRABANDO AUDIO CON MEJORAS DE UX */
        <div className={`rounded-2xl border p-10 text-center flex flex-col items-center justify-center space-y-6 transition-all ${
          isNearLimit ? 'border-amber-300 bg-amber-50/20' : 'border-emerald-300/60 bg-emerald-50/10'
        }`}>
          
          {/* Badge dinámico con tiempo y alerta de límite */}
          <div className={`inline-flex items-center gap-2.5 px-4 py-2 rounded-full text-xs font-semibold ${
            isNearLimit ? 'bg-amber-100 text-amber-800' : 'bg-[#D1FADF]/70 text-[#027A48]'
          }`}>
            <svg className="w-4 h-4 animate-pulse" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z" />
            </svg>
            <span>{isPaused ? 'GRABACIÓN PAUSADA' : 'GRABANDO AUDIO'}</span>
            <span className="border-l border-current/20 pl-2.5 font-mono text-xs">
              {formatTime(recordingSeconds)} / {formatTime(maxSeconds)}
            </span>
          </div>

          {/* Ondas de audio simuladas en vivo */}
          {!isPaused && (
            <div className="flex items-center justify-center gap-1 h-6">
              <span className="w-1 bg-[#12B76A] rounded-full animate-bounce h-3"></span>
              <span className="w-1 bg-[#12B76A] rounded-full animate-bounce h-6 [animation-delay:0.2s]"></span>
              <span className="w-1 bg-[#12B76A] rounded-full animate-bounce h-2 [animation-delay:0.4s]"></span>
              <span className="w-1 bg-[#12B76A] rounded-full animate-bounce h-5 [animation-delay:0.1s]"></span>
              <span className="w-1 bg-[#12B76A] rounded-full animate-bounce h-4 [animation-delay:0.3s]"></span>
            </div>
          )}

          {/* Botones de control */}
          <div className="flex items-center justify-center gap-8 py-2">
            
            {/* Botón Pausar */}
            <div className="flex flex-col items-center gap-2">
              <button
                onClick={isPaused ? onResumeRecording : onPauseRecording}
                className="flex h-14 w-14 items-center justify-center rounded-full bg-[#334155] text-white shadow-md hover:bg-[#1E293B] transition-all"
              >
                {isPaused ? (
                  <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M8 5v14l11-7z" />
                  </svg>
                ) : (
                  <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                    <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" />
                  </svg>
                )}
              </button>
              <span className="text-xs font-medium text-slate-500">
                {isPaused ? 'Reanudar' : 'Pausar'}
              </span>
            </div>

            {/* Botón Finalizar Nota con doble confirmación anti-clic */}
            <div className="flex flex-col items-center gap-2">
              <button
                onClick={handleStopClick}
                className={`flex items-center justify-center rounded-full text-white shadow-lg transition-all ${
                  confirmingStop 
                    ? 'h-20 px-6 bg-amber-600 hover:bg-amber-700 text-xs font-bold animate-pulse' 
                    : 'h-20 w-20 bg-[#FF2E55] hover:bg-[#E02447] shadow-red-500/25'
                }`}
              >
                {confirmingStop ? (
                  '¿Confirmar?'
                ) : (
                  <div className="h-6 w-6 rounded-xs bg-white"></div>
                )}
              </button>
              <span className="text-xs font-bold text-[#FF2E55]">
                {confirmingStop ? 'Toca de nuevo' : 'Finalizar Nota'}
              </span>
            </div>

          </div>

          {/* Mensaje inferior */}
          <div className="space-y-1">
            <p className="text-xs text-slate-500 font-medium">
              {isPaused ? 'Consulta en pausa. Retoma cuando estés listo.' : 'Habla con naturalidad. MedikAI está capturando los hallazgos.'}
            </p>
          </div>

        </div>
      )}
    </div>
  );
}